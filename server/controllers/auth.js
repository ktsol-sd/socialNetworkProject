...
export const login = async (req, res) => {
  ...
  try {
    ...
    // check if the db has user with the email given
...
export const forgotPassword = async (req, res) => {
  ...
  if (!newPassword || newPassword.length < 6) {
    return res.json({
      error: "New password is required and should be more than 6 characters",
    });
  }
  if (!secret) {
    return res.json({
...
export const searchUser = async (req, res) => {
  ...
  if (!query) return;
  try {
    //i is a modifier used for case sensitive matching
